...
# ROE is quarterly, so ffill then shift to align to daily and prevent look-ahead
quality_roe = roe.rolling(4).mean().ffill().shift(1)
# Factor 3: Growth (Monthly Revenue YoY)
# Revenue is monthly, so ffill then shift
...
# Factor 5: RSI Reversal (Lower RSI implies oversold, potential reversal)
# RSI is daily, just shift
...
# For RSI reversal, lower RSI is better, so we invert the rank to give higher scores to lower RSI values
rsi_reversal_rank = 1 - rsi_reversal.rank(axis=1, pct=True)
# Combine factors with intelligent weights
# Momentum and Quality/Growth are strong positive signals
# Institutional flow confirms trend/sentiment
# RSI reversal adds a contrarian/value component
...
# Market Cap filter: Market Cap > 10 Billion TWD (ffill for market_cap as it's not daily)
