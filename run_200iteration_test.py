#!/usr/bin/env python3
"""
200-Iteration Production Validation Test Run

Runs 200 iterations of the autonomous learning loop using ExtendedTestHarness
with automatic checkpointing, retry logic, and comprehensive statistical analysis.

This is an extended validation run requested by the user to verify long-term
stability of Phase 1 + Phase 2 implementations.

Usage:
    python3 run_200iteration_test.py [group_id]

Arguments:
    group_id: Optional identifier for test group (default: 1)
"""
...
def setup_logging(group_id: str = "1"):
    ...
    Args:
        group_id: Test group identifier (default: "1")
    ...
    log_file = os.path.join(log_dir, f"200iteration_test_group{group_id}_{timestamp}.log")
...
def print_colored_report(report: dict, logger: logging.Logger, metrics: dict = None, group_id: str = "1"):
    ...
    logger.info("")
    logger.info("=" * 80)
    logger.info(f"200-ITERATION TEST - GROUP {group_id} - PRODUCTION READINESS REPORT")
    logger.info("=" * 80)
...
def run_200iteration_test(group_id: str = "1", resume_from: str = None):
    """Run 200-iteration test with ExtendedTestHarness.

    Main test orchestration function that:
    - Sets up logging
    - Loads Finlab data
    - Initializes ExtendedTestHarness
    - Executes 200-iteration test run
    ...
    logger.info("=" * 80)
    logger.info(f"200-ITERATION PRODUCTION TEST - GROUP {group_id} - START")
    logger.info("=" * 80)
    logger.info(f"Timestamp: {datetime.now().isoformat()}")
    logger.info(f"Model: gemini-2.5-flash (Primary: Google AI, Fallback: OpenRouter)")
    logger.info(f"Target iterations: 200")
    logger.info(f"Checkpoint interval: 20")
    ...
    try:
        checkpoint_dir = f'checkpoints_group{group_id}'
    ...
    logger.info("")
    logger.info(f"Starting 200-iteration test run (Group {group_id})...")
    logger.info("")
    ...
    try:
        ...
        logger.info("")
        logger.info("=" * 80)
        logger.info(f"TEST EXECUTION SUMMARY - GROUP {group_id}")
        logger.info("=" * 80)
        ...
        if statistical_report and not statistical_report.get('error'):
            print_colored_report(statistical_report, logger, metrics, group_id)
        else:
...
def main():
    """Main entry point for 200-iteration production test.

    Handles:
    - Command-line argument parsing for group ID and checkpoint resume
    ...
    try:
        ...
        print(f"\n🚀 Starting 200-iteration production test (Group {group_id})...\n")

        results = run_200iteration_test(group_id=group_id, resume_from=resume_from)

        if results.get('success'):
            statistical_report = results.get('statistical_report', {})
            production_ready = statistical_report.get('production_ready', False)

            print(f"\n✅ Test completed successfully (Group {results['group_id']})")
            ...
            print(f"\n❌ Test failed (Group {results.get('group_id', 'unknown')}): {results.get('error', 'Unknown error')}")
            print(f"   Log file: {results['log_file']}")
            sys.exit(1)
        ...
        print(f"  python run_200iteration_test.py {group_id} checkpoints_group{group_id}/checkpoint_iter_<N>.json")
        print("\nNote: Checkpoints are saved every 20 iterations")
        sys.exit(2)
