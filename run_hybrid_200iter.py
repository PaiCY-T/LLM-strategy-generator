#!/usr/bin/env python3
"""
200-Iteration Hybrid Test - Innovation Rate Validation
Tests innovation_rate=50.0 (50% LLM, 50% Factor Graph)
"""
import os
import sys
...
print("=" * 80)
print("200-Iteration Hybrid Test (innovation_rate=50.0)")
print("Expected: ~100 LLM iterations + ~100 FG iterations")
print("=" * 80)
print()
...
print("Initializing UnifiedTestHarness (Hybrid mode: FG/LLM = 50/50)...")
harness = UnifiedTestHarness(
    model='gemini-2.5-flash',
    target_iterations=200,
    template_mode=False,  # Hybrid mode
    use_json_mode=False,  # Correct: template_mode=False incompatible with use_json_mode=True
    innovation_rate=50.0,  # Hybrid mode: 50% LLM, 50% Factor Graph
...
print("✓ Harness initialized (innovation_rate=50.0 = Hybrid Mode: 50% FG / 50% LLM)\n")

# Run test
print("Starting 200-iteration hybrid test...")
print(f"Start time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
print(f"Expected: ~100 FG iterations + ~100 LLM iterations")
print(f"Estimated duration: ~35-45 minutes")
print()
...
try:
    ...
    expected_llm = 50.0
    tolerance = 10.0  # ±10% for 200 iterations
...
# Display results
print("\n" + "=" * 80)
print("RESULTS - HYBRID 200 ITERATIONS (FG/LLM = 50/50)")
print("=" * 80)
...
output_file = f"results/hybrid_200iter_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
