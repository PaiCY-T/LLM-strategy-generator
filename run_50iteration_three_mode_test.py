#!/usr/bin/env python3
"""
150-Iteration Three-Mode Test (50 iterations × 3 modes)

Runs 50 iterations each across THREE strategy generation modes:
1. Factor Graph Only Mode (innovation_rate=0%)
2. LLM Only Mode (innovation_rate=100%)
3. Hybrid Mode (innovation_rate=50%)

Total: 150 iterations to validate multiprocessing pickle fix

Usage:
    python3 run_50iteration_three_mode_test.py
"""
...
def setup_logging(base_output_dir: str) -> logging.Logger:
    ...
    log_file = log_dir / f"150iteration_three_mode_test_{timestamp}.log"
...
def run_mode_test(
    ...
    """Run test for a specific generation mode (50 iterations from config file).
    ...
    try:
        ...
        logger.info("(Estimated time: 8-15 minutes per 50 iterations)")
        logger.info("")
...
def main():
    """Run 150-iteration three-mode validation test."""
    # Setup
    base_output_dir = "experiments/llm_learning_validation/results/50iteration_three_mode"
    logger = setup_logging(base_output_dir)

    logger.info("")
    logger.info("=" * 80)
    logger.info("150-ITERATION THREE-MODE VALIDATION TEST")
    logger.info("=" * 80)
    ...
    logger.info("  1. Factor Graph Only: 50 iterations (0% LLM)")
    logger.info("  2. LLM Only: 50 iterations (100% LLM)")
    logger.info("  3. Hybrid: 50 iterations (50% LLM + 50% FG)")
    logger.info("  TOTAL: 150 iterations")
    logger.info("")
    logger.info("Purpose: Validate multiprocessing pickle fix stability")
    logger.info("")
    ...
    test_modes = [
        {
            ...
            "config": "experiments/llm_learning_validation/config_fg_only_50.yaml"
        },
        {
            "name": "LLM Only",
            "config": "experiments/llm_learning_validation/config_llm_only_50.yaml"
        },
        {
            "name": "Hybrid",
            "config": "experiments/llm_learning_validation/config_hybrid_50.yaml"
        }
    ]
    ...
    comprehensive_results = {
        "test_name": "150-iteration-three-mode-validation-test",
    ...
    logger.info(f"Total Iterations: 150 (50 × 3 modes)")
    logger.info("")
