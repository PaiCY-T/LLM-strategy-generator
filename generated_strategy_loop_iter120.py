...
# ROE is quarterly, so ffill to daily, then smooth, then shift
...
# P/E is quarterly/monthly, so ffill to daily, then inverse, then shift
...
# P/E Ratio filter: Avoid extremely high P/E (overvalued) and extremely low P/E (distressed)
pe_upper_filter = pe_ratio.ffill().shift(1) < 30
pe_lower_filter = pe_ratio.ffill().shift(1) > 5
# Apply all filters
filtered_factor = combined_factor[liquidity_filter & price_filter & pe_upper_filter & pe_lower_filter]
# 5. Select stocks
