...
# Factor 5: RSI (oversold avoidance)
rsi_signal = (rsi.shift(1) < 30).astype(int) # 1 if oversold, 0 otherwise
...
rsi_rank = rsi_signal.rank(axis=1, pct=True) # Rank the oversold signal
# Assign weights. Higher weight to momentum, quality, and revenue. Institutional flow and RSI get smaller weights.
...
# RSI filter: Avoid extremely overbought conditions
rsi_overbought_filter = rsi.shift(1) < 70
# Combine all filters
filtered_factor = combined_factor[liquidity_filter & price_filter & rsi_overbought_filter]
# 5. Select stocks
