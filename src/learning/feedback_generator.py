...
class FeedbackGenerator:
    ...
    def __init__(self, history, champion_tracker):
    ...
    def _select_template_and_variables(
        ...
        # Execution error
        if context.execution_result.get('status') == 'error':
        ...
        if context.metrics and len(recent_records) >= 2:
            ...
            prev_sharpe = prev_record.metrics.get('sharpe_ratio', 0)
        ...
        return TEMPLATE_SUCCESS_SIMPLE, {
            'iteration_num': context.iteration_num,
            'current_sharpe': context.metrics.get('sharpe_ratio', 0) if context.metrics else 0,
            'classification_level': context.classification_level or 'N/A',
            'champion_section': self._format_champion_section(context.metrics.get('sharpe_ratio', 0) if context.metrics else 0)
        }

    def _analyze_trend(self, recent_records: List) -> str:
    ...
    def _find_last_success(self, recent_records: List) -> Optional[Any]:
    ...
    def _format_last_success(self, last_success: Optional[Any]) -> str:
        ...
        sharpe = last_success.metrics.get('sharpe_ratio', 0)
