#!/usr/bin/env python3
"""
Phase 1: Smoke Test (10 Generations)

Quick validation of population-based evolutionary learning with:
- Smaller population (30) for speed
- 10 generations for rapid testing
- Tournament selection with elitism
- Adaptive mutation rates
- IS/OOS data split validation

Target Metrics (Quick Validation):
- Champion update rate: ≥10% (1+ updates in 10 generations)
- Best In-Sample Sharpe: >2.5
- Champion Out-of-Sample Sharpe: >1.0
...
def setup_logging():
    ...
    log_file = os.path.join(log_dir, f"phase1_smoke_test_{timestamp}.log")
...
def run_phase1_smoke_test():
    """Run 10-generation smoke test for Phase 1 population-based learning.

    Configuration:
    - Population size: 30 (reduced for speed)
    - Elite size: 3 (10%)
    - Generations: 10
    ...
    logger.info("=" * 70)
    logger.info("PHASE 1: SMOKE TEST (10 GENERATIONS)")
    logger.info("=" * 70)
    logger.info(f"Timestamp: {datetime.now().isoformat()}")
    logger.info(f"Mode: Population-based evolutionary learning")
    logger.info(f"Target generations: 10")
    logger.info(f"Population size: 30 (quick validation)")
    logger.info(f"Expected duration: 25-30 minutes")
    ...
    # Initialize Phase1TestHarness for smoke test
    logger.info("Initializing Phase1TestHarness for smoke test...")
    ...
    # Run test
    try:
        logger.info("Starting 10-generation smoke test...")
        logger.info("")
        ...
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        results_file = os.path.join(results_dir, f"phase1_smoke_test_{timestamp}.json")
...
def main():
    """Main entry point."""
    try:
        print("\n🚀 Starting Phase 1 smoke test (10 generations)...\n")
        print("⏱️  Estimated duration: 25-30 minutes\n")

        results = run_phase1_smoke_test()
        ...
        if results.get('success'):
            ...
            if champion_update_rate >= 10.0 and best_is_sharpe > 2.5 and champion_oos_sharpe > 1.0:
                print("   ✅ SUCCESS: All targets met in smoke test")
                print("   📝 Recommendation: Proceed to full test (50 generations)")
            elif champion_update_rate >= 5.0 and best_is_sharpe > 2.0 and champion_oos_sharpe > 0.6:
                print("   ⚠️  PARTIAL: Promising results")
                print("   📝 Recommendation: Proceed to full test for validation")
            else:
                print("   ❌ NEEDS IMPROVEMENT: Below expectations")
                print("   📝 Recommendation: Review configuration before full test")
