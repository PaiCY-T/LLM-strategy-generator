...
# ROE is quarterly, so forward-fill then calculate rolling mean, then shift
...
# Foreign net buy is daily, so calculate rolling sum then shift
...
combined_factor = (momentum_rank * 0.30 +
                   ...
                   flow_rank * 0.20)
# 4. Apply filters
# Mandatory Liquidity filter (average trading value > 50M TWD over 20 days)
liquidity_filter = trading_value.rolling(20).mean().shift(1) > 50_000_000
# Mandatory Price filter (close price > 15 TWD)
price_filter = close.shift(1) > 15
# Optional Market Cap filter (market cap > 10B TWD for mid-large cap)
market_cap_filter = market_cap.shift(1) > 10_000_000_000
# Optional RSI filter (RSI between 30 and 70, avoiding extreme overbought/oversold)
