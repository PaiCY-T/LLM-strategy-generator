...
# Use .ffill() for quarterly data and then .shift(1)
quality = roe.rolling(4).mean().ffill().shift(1)
# Factor 3: Growth (Monthly Revenue YoY)
# Use .ffill() for monthly data and then .shift(1)
...
# Handle potential division by zero or inf values by letting rank handle them
...
# Market Cap filter: Market capitalization > 10B TWD (focus on mid-large cap)
