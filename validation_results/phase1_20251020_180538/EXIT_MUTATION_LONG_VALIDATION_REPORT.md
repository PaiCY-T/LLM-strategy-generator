# Exit Mutation Long-Running Validation Report

**Generated**: 2025-10-20T18:05:39.887950

## Executive Summary

**Decision**: ⚠️  OPTIMIZATION NEEDED - Address stability/success rate issues

### Phase 2a Readiness Criteria

- ❌ FAIL: Sharpe Target
- ❌ FAIL: Diversity Target
- ❌ FAIL: Success Rate Target
- ❌ FAIL: Stability Ok
- ✅ PASS: Performance Maintained

## Performance Analysis

- **Initial Sharpe**: 1.6966
- **Final Sharpe**: 1.6966
- **Best Sharpe**: 1.6966
- **Improvement**: +0.0000
- **Avg Improvement Rate**: 0.0000/gen
- **Plateaued**: No ✅

### Sharpe Ratio Progression

```
Gen  0: 1.6966 █████████████████████████████████
```

## Diversity Analysis

- **Unique Exit Patterns**: 0
- **Jaccard Diversity**: 0.000
- **Exit Patterns Found**:

## Exit Mutation Analysis

- **Total Attempts**: 0
- **Successes**: 0
- **Failures**: 0
- **Success Rate**: 0.00%

### Mutation Types Distribution

- Parametric: 0
- Structural: 0
- Relational: 0

## Stability Analysis

- **Total Issues**: 4

### Issues Detected

- **Gen 1**: 'NoneType' object has no attribute 'sharpe_ratio'
- **Gen 2**: 'NoneType' object has no attribute 'sharpe_ratio'
- **Gen 3**: 'NoneType' object has no attribute 'sharpe_ratio'
- **Gen 4**: 'NoneType' object has no attribute 'sharpe_ratio'

## Generation Breakdown

| Gen | Best Sharpe | Mutations | Success Rate | Patterns | Time |
|-----|-------------|-----------|--------------|----------|------|

## Decision Recommendation

**Recommendation**: ⚠️  OPTIMIZATION NEEDED - Address stability/success rate issues

### Next Steps

1. Address exit mutation success rate issues
2. Fix stability problems before proceeding
3. Re-run validation after fixes
4. Consider parameter tuning for mutation operator

## Timing Summary

- **Total Time**: 0.0 minutes
- **Avg Generation Time**: 0.0s
- **Generations Completed**: 0/10
