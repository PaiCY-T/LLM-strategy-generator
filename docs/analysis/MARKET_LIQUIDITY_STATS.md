# Taiwan Stock Market Liquidity Statistics

**Generated**: 2025-10-10 08:21:37
**Analysis Period**: 60-day rolling average
**Total Stocks Analyzed**: 2,632

---

## Overall Market Statistics

Stock availability at different liquidity thresholds:

| Threshold (TWD) | Stock Count | % of Market | Portfolio Impact |
|----------------|-------------|-------------|------------------|
|     50M |         803 |       30.5% | Low - Very limited selection |
|    100M |         599 |       22.8% | Low - Very limited selection |
|    150M |         480 |       18.2% | Very Low - Severe constraints |
|    200M |         413 |       15.7% | Very Low - Severe constraints |

---

## Market Cap Breakdown

### Market Cap Categories

- **Large Cap**: Market value > 100 billion TWD (>100億)
- **Mid Cap**: Market value 10-100 billion TWD (10億-100億)
- **Small Cap**: Market value < 10 billion TWD (<10億)

### Large Cap Stocks

| Threshold (TWD) | Stock Count | % of Large Cap |
|----------------|-------------|----------------|
|     50M |         108 |         100.0% |
|    100M |         107 |          99.1% |
|    150M |         105 |          97.2% |
|    200M |         102 |          94.4% |

### Mid Cap Stocks

| Threshold (TWD) | Stock Count | % of Mid Cap |
|----------------|-------------|--------------|
|     50M |         356 |       100.0% |
|    100M |         281 |        78.9% |
|    150M |         231 |        64.9% |
|    200M |         204 |        57.3% |

### Small Cap Stocks

| Threshold (TWD) | Stock Count | % of Small Cap |
|----------------|-------------|----------------|
|     50M |         191 |         100.0% |
|    100M |         110 |          57.6% |
|    150M |          74 |          38.7% |
|    200M |          54 |          28.3% |

---

## Portfolio Construction Implications

### Key Insights

1. **150M TWD Threshold (Recommended)**:
   - 480 stocks available (18.2% of market)
   - ❌ Low liquidity - severe portfolio constraints
   - ❌ May need to lower threshold or reduce position count

2. **Market Cap Distribution**:
   - Large cap: 105 stocks (25.6%)
   - Mid cap: 231 stocks (56.3%)
   - Small cap: 74 stocks (18.0%)
   - 💡 Good balance of mid-cap opportunities

3. **Threshold Selection Guidance**:

   - **50M TWD**: 803 stocks (30.5%) - Very liquid, may include volatile small caps
   - **100M TWD**: 599 stocks (22.8%) - Good balance of liquidity and quality
   - **150M TWD**: 480 stocks (18.2%) - Recommended for monthly rebalancing
   - **200M TWD**: 413 stocks (15.7%) - Very high quality, reduced selection

### Recommendations

- **For weekly rebalancing**: Use 200M TWD threshold (higher quality, lower impact)
- **For monthly rebalancing**: Use 150M TWD threshold (balanced approach)
- **For large portfolios (>20 stocks)**: Use 100M TWD threshold (wider selection)
- **Market cap constraints**: Consider separate thresholds per cap category

### Data Quality Notes

- Analysis based on 60-day rolling average trading value
- Market cap data from most recent available snapshot
- Stocks with incomplete data excluded from analysis
- Thresholds should be reviewed periodically as market conditions change
