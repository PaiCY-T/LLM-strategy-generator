...
# Factor 5: RSI (oversold/overbought signal)
...
rsi_rank = rsi_signal.rank(axis=1, pct=True) # Rank RSI signal
...
flow_weight = 0.15
rsi_weight = 0.1
combined_factor = (momentum_rank * momentum_weight +
                   revenue_rank * revenue_weight +
                   quality_rank * quality_weight +
                   flow_rank * flow_weight +
                   rsi_rank * rsi_weight)
# 4. Apply filters
# Liquidity filter: Average daily trading value over last 20 days > 40M TWD
...
# RSI filter: Avoid extremely overbought or oversold conditions for selection
