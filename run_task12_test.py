#!/usr/bin/env python3
"""
Standalone Task 12 test runner.
Runs the main Task 12 test without pytest infrastructure.
"""
...
""",
    ...
    # Configure LLM via YAML config mock (yaml is imported inside _initialize_llm)
    # The yaml.safe_load is called twice: once for sandbox config, once for LLM config
    # We return the same config for both calls
    llm_config = {
        'llm': {
            'enabled': 'true',
            'provider': 'gemini',
        ...
        'sandbox': {
            'enabled': False  # Disable sandbox for faster testing
        }
    }

    with patch('yaml.safe_load') as mock_yaml:
        mock_yaml.return_value = llm_config
