...
# Factor 4: Technical Strength (RSI, avoiding extremes)
# Use RSI directly as a strength indicator, higher RSI (within bounds) is better
tech_strength = rsi.shift(1)
# 3. Combine factors (normalize first!)
momentum_rank = momentum.rank(axis=1, pct=True)
quality_rank = quality.rank(axis=1, pct=True)
flow_rank = institutional_flow.rank(axis=1, pct=True)
tech_strength_rank = tech_strength.rank(axis=1, pct=True)
# Combine with meaningful weights
combined_factor = (momentum_rank * 0.35 +
                   quality_rank * 0.25 +
                   flow_rank * 0.25 +
                   tech_strength_rank * 0.15)
# 4. Apply filters
...
# Market cap might not be daily, so ffill is used
